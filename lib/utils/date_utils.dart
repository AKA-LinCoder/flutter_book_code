...
class EchoDateUtils {
  ...
  static String formatDateMs(int ms, {bool isUtc = false, String? format}) {
    return formatDate(getDateTimeByMs(ms, isUtc: isUtc), format: format??defaultDateFormatDate);
  }
  ...
  static String formatDateStr(String dateStr, {bool? isUtc, String? format}) {
    return formatDate(getDateTime(dateStr, isUtc: isUtc), format: format??defaultDateFormatDate);
  }
  ...
  static bool isToday(int? milliseconds, {bool isUtc = false, int? locMs}) {
    ...
    DateTime now;
    if (locMs != null) {
      now = getDateTimeByMs(locMs)??DateTime.now();
    } else {
  ...
  static bool isWeek(int? ms, {bool isUtc = false, int? locMs}) {
    ...
    DateTime _now;
    if (locMs != null) {
      _now = getDateTimeByMs(locMs, isUtc: isUtc)??DateTime.now();
    } else {
